...
{
    public async Task<EndSessionResultDto> Handle(EndExamSessionCommand request, CancellationToken ct)
    {
        ...
        // Need NegativeMarkValue from paper
        var paper = await academicApi.GetPaperSummaryAsync(session.PaperId!.Value, ct)
            ?? throw new NotFoundException("Paper not found.");

        // Score all responses — raises ExamSessionCompletedDomainEvent
        var score = session.Complete(paper.NegativeMarkValue);
        ...
        int timeTaken     = session.Duration.HasValue ? (int)session.Duration.Value.TotalSeconds : 0;
